...
`commadapter.vehicle.vda5050.orderResendTimeoutMs`::
未确认订单再次发送前的最小间隔（以毫秒为单位）。
设置为 `0` 时保留旧行为，即每收到一帧 state 都可以重试，直到订单被确认。
...
commadapter.vehicle.vda5050.orderResendTimeoutMs = 1000
