...
The minimum interval (in ms) before an unacknowledged order is resent.
Set to `0` to keep the legacy behaviour of retrying on every state message until the order is acknowledged.
